...
const {
  ...
  max = 0,
  bloodiedThresh = 0.25,
} = defineProps<{ current?: number; max?: number; bloodiedThresh?: number }>();

const isDead = computed(() => current <= 0);
const isBloodied = computed(() => !isDead.value && max! * bloodiedThresh >= current!);
...
<template>
  <div class="pulse" :class="classObject" :style="styleObject"></div>
</template>
...
@keyframes flat {
  ...
  10% {
    clip-path: rect(0 20% $h 0);
  }
  40% {
    clip-path: rect(0 $w $h #{calc($w * 0.8)});
  }
  50% {
    clip-path: rect(0 $w $h $w);
  }
